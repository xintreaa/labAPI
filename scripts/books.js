    ...
    try {
        ...
        books.forEach(book => {
            ...
            row.innerHTML = `
                ...
                <td>${(book.authors && Array.isArray(book.authors) && book.authors.length) ? book.authors.map(a => a.first_name + ' ' + a.last_name).join(', ') : 'No authors'}</td>
                <td>${(book.categories && Array.isArray(book.categories) && book.categories.length) ? book.categories.map(c => c.name).join(', ') : 'No categories'}</td>
...
async function handleSubmit(event) {
    ...
    if (isNaN(publicationYear) || isNaN(quantity) || quantity < 0) {
        alert('Please enter valid year and quantity (non-negative numbers).');
        return;
    }
...
async function editBook(id) {
    try {
        ...
        document.getElementById('author-ids').value = (book.authors && Array.isArray(book.authors) && book.authors.length) ? book.authors.map(a => a.id || '').join(', ') : '';
        document.getElementById('category-ids').value = (book.categories && Array.isArray(book.categories) && book.categories.length) ? book.categories.map(c => c.id || '').join(', ') : '';
        ...
        alert('Failed to fetch book for editing: ' + (error.response?.data?.detail || error.message || 'Unknown error'));
    }
}
